...
void add_event_to_hashmap(Midi *midi, u64 delta_time, EventValue event_value);
...
void init_midi(Midi *midi) {
  // *midi = (Midi){0};
  // midi->capacity = 128;
  // midi->events = (MidiEvent *)malloc(sizeof(MidiEvent) * 128);
  // assert(midi->events != NULL);
}

void rehash(Midi *midi) {
  // midi->capacity = midi->capacity * 2;
  // MidiEvent *temp = malloc(midi->capacity * sizeof(*midi->events));
  // assert(temp != NULL);
  // MidiEvent *temp2 = midi->events;
  // u32 length = midi->size;
  // midi->events = temp;
  //
  // // loop through and transfer the event from temp2 to midi events this
  // action
  // // is destructive
  // midi->size = 0;
  ...
  //       add_event_to_hashmap(midi, temp2[i].delta_time,
  //                            temp2[i].event_value[count]);
  //     }
  //   } else {
  //     add_event_to_hashmap(midi, temp2[i].delta_time,
  //     temp2[i].event_value[0]);
  //   }
  // }
  // free(temp2);
}
void append_event(MidiEvent *midi_event, EventValue event_value) {
  // midi_event->size += 1;
  // EventValue *temp =
  //     realloc(midi_event, midi_event->size * sizeof(event_value));
  // assert(temp != NULL);
  // EventValue *temp2 = midi_event->event_value;
  // midi_event->event_value = temp;
  // midi_event->event_value[midi_event->size - 1] = event_value;
  // free(temp2);
}
...
void add_event_to_hashmap(Midi *midi, u64 delta_time, EventValue event_value) {
  ...
  // }
  //
  // int hash = hash_int(delta_time, midi->capacity);
  // if (midi->events[hash].size > 0) {
  //   printf("delta time: %llx\n", delta_time);
  //   printf("midi event delta time: %llx\n", midi->events[hash].delta_time);
  //   assert(delta_time == midi->events[hash].delta_time);
  //   append_event(midi->events, event_value);
  // } else {
  //   midi->events[hash].event_value = (EventValue
  //   *)malloc(sizeof(EventValue)); midi->events[hash].delta_time = delta_time;
  //   midi->events[hash].event_value[0] = event_value;
  //   midi->events[hash].size = 1;
  //   midi->size++;
  // }
}
...
u64 file_read_byte(int size, FILE *file) {
...
u64 file_read_variable_length(FILE *file) {
  u8 buffer;
  unsigned int len = sizeof(u8);
  u64 temp = 0;
  u32 count = 0;
  ...
  if ((status & 0xF0) == 0xF0) {
    if (status == 0xFF) {
      ...
      case TEXT_EVENT: {
        ...
        EventValue event_value = {
            .event_type = META_EVENT, .event_id = type, .value.text = data};
      ...
      case SEQUENCE_NUMBER: {
        ...
        EventValue event_value = {
            .event_type = META_EVENT, .event_id = type, .value.number = number};
        add_event_to_hashmap(&midi, delta_time, event_value);
      ...
      case END_OF_TRACK: {
        ...
        assert(length == 0x00);
        EventValue event_value = {
            .event_type = META_EVENT, .event_id = type, .value = {0}};
        add_event_to_hashmap(&midi, delta_time, event_value);
      ...
      case SET_TEMPO: {
        ...
        EventValue event_value = {
            .event_type = META_EVENT, .event_id = type, .value.tempo = tempo};
        add_event_to_hashmap(&midi, delta_time, event_value);
  ...
  while ((ftell(midi_file) < file_len)) {
    ...
    if (strcmp(identifier, "MTrk") == 0) {
      ...
      while ((ftell(midi_file) < chunk_end)) {
        ...
        // any byte & 0x80 results in 0 if the byte is < 0x80 and results in
        // 0x80 if the byte is greater than 0x80. we use 0x80 because it's a
        // byte that has 1 as the MSB and the rest bytes is 0
        if (next_byte & 0x80) {
